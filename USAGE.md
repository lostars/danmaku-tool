...
该部分配置主要提升搜索和匹配的精准度，可自行阅读说明进行配置。
具体配置可以参考 [example](configs/config.example.yaml)

> 场景1

由于国内各大流媒体都是类似 album:video 这种模式来组织剧集，所以基本都将第几季信息放在了剧集标题上，比如 xxx第几季。

`danmaku-tool` 默认使用搜索结果的标题来匹配出季信息。但是总有搜索结果标题是不带季信息的，比如 `刺客伍六七`，
B站使用标题 `伍六七之xxx` 类似标题，导致无法准确匹配，且 `刺客` 这个词在挺多平台都是黑名单，搜索结果是不会返回该词的。

`tokenizer.blacklist` 配置则是用于处理上面这种情况。

> 场景2

`danmaku-tool` 在配置了元数据提供商后（比如Emby），会从元数据获取年份信息用于提升匹配准确度，
但是总有情况流媒体平台的年份信息和元数据是对不上的。

`tokenizer.year-match-list` 则是用于配置某些剧集的年份信息，相当于手动重新给剧集配置了年份信息。

> 场景3

1. 流媒体平台集数信息和元数据对不上，常见的就是S0的剧集，国内流媒体可能会把S0的剧集放在某一季最后；
2. 流媒体平台一部剧集对应元数据多部剧集，比如腾讯的火影忍者是720集的合订本，且集数缺失；

`tokenizer.rematch` 支持两种方式来重新组织流媒体平台的剧集数据：
1. 将流媒体一部剧集直接映射到元数据多部剧集，按照配置顺序。
2. 手动配置流媒体一部剧集，支持 `1-10,11,12` 范围、单集混合配置。
